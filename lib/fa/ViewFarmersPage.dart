...
class _ViewFarmersPageState extends State<ViewFarmersPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _setupSharedPreferencesListener();
  }
  ...
  void _setupSharedPreferencesListener() {
    SharedPreferences.getInstance().then((prefs) async {
      prefs.reload();
      String? userName = prefs.getString('userName');
      bool userNameChanged = prefs.getBool('userNameChanged') ?? false;

      if (userNameChanged) {
        await prefs.setBool('userNameChanged', false);
        await prefs.remove('farmers_data'); // Force refetch
        _loadFarmersData();
      }
      ...
      prefs.setString('lastKnownUserName', userName ?? '');
    });
    ...
    Future.doWhile(() async {
      ...
      String? lastKnownUserName = prefs.getString('lastKnownUserName');
      if (newUserName != lastKnownUserName) {
        print('Username changed from $lastKnownUserName to $newUserName');
        prefs.setString('lastKnownUserName', newUserName ?? '');
        _loadFarmersData();
      }
      return true;
    });
  ...
  Future<void> _loadFarmersData() async {
    ...
    if (userName != currentUserName || userNameChanged) {
      setState(() {
        currentUserName = userName;
      });
      storedFarmers = null; // Force refetch if username changed
      if (userNameChanged) {
        await prefs.setBool('userNameChanged', false);
      }
    }
  ...
  Future<void> _fetchDataFromFirestore() async {
    try {
      ...
      List<Map<String, dynamic>> fetchedFarmers = querySnapshot.docs.map((doc) {
        ...
        data['Area'] = (data['Area'] as num?)?.toInt() ?? 0;
        return data;
      }).toList();
  ...
  Future<void> _showEditDialog(Map<String, dynamic> farmer, String column) async {
    if (column == 'Date of TP') {
      ...
      try {
        currentDate = DateFormat('dd/MM/yyyy').parse(farmer[column] ?? '');
      } catch (e) {
      ...
      if (newDate != null) {
        setState(() {
          farmer[column] = DateFormat('dd/MM/yyyy').format(newDate);
        });
        _updateLocalStorage();
      ...
      if (newValue != null && newValue != farmer[column]?.toString()) {
        setState(() {
          if (column == 'Area') {
            farmer[column] = int.tryParse(newValue) ?? 0;
          } else if (column == 'Farmer Name' || column == 'Village') {
            farmer[column] = _capitalizeWords(newValue);
          } else {
  ...
  Future<void> _showAddFarmerDialog() async {
    ...
    bool isDavanagereNursery = false;
    ...
    SizedBox(height: 8),
    Text('Date of TP', style: TextStyle(fontWeight: FontWeight.bold)),
    CupertinoButton(
    ...
    children: [
    Text('Davanagere Nursery'),
    SizedBox(width: 8),
    CupertinoSwitch(
    value: isDavanagereNursery,
    onChanged: (bool value) {
    setState(() {
    isDavanagereNursery = value;
    });
    },
    ),
    ],
          ...
          actions: [
            ...
            CupertinoDialogAction(
              child: Text('Save'),
              onPressed: () {
                if (name.isNotEmpty && village.isNotEmpty && area > 0) {
                  _addNewFarmer(name, village, area, dateOfTP, isDavanagereNursery);
  ...
  void _addNewFarmer(String name, String village, int area, DateTime dateOfTP, bool isDavanagereNursery) {
    Map<String, dynamic> newFarmer = {
      'Farmer Name': _capitalizeWords(name),
      'Village': _capitalizeWords(village),
      'Area': area,
      'Date of TP': DateFormat('dd/MM/yyyy').format(dateOfTP),
    };

    if (isDavanagereNursery) {
      newFarmer['Nursery'] = '`Davanagere Nursery`';
    }
      ...
      child: Container(
        ...
        child: SafeArea(
          child: isLoading
              ...
              ? Center(
            child: Text(
              'No farmers are added yet.\nAdd new farmers by clicking on the plus icon.',
